...
//  sparqlList:any[];
  ...
  //        this.isSparql0 = false;
      ...
      */
            ...
            if (this.claims.P189 !== undefined) { // Pictures from Wikimedia Commons
            
              this.pictures = this.claims.P189.map(picture => {
                //    const imageUrl = picture.mainsnak.datavalue.value;
                const imageUrl = picture.picture;
                const thumbnailUrl = `${imageUrl}?width=300`;
                console.log(imageUrl);

                this.preloadImage(thumbnailUrl); // Précharge la miniature

                // Précharger l'image
                this.preloadImage(imageUrl);

                return {
                  thumbnail: `${imageUrl}?width=300`, // Miniature pour affichage rapide
                  full: imageUrl // URL complète
                };
              });
            }
  ...
  private preloadImage(url: string): void {
    console.log('URL passed to preloadImage:', url); // Vérification de l'URL
    const link = document.createElement('link');
    link.rel = 'preload';
    link.as = 'image';
    link.href = url;
    console.log('Link element created:', link); // Vérification de l'élément link
