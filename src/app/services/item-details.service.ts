  ...
  addClaimItemDetails(items, re, itemProperties, lang) {
    for (let i = 0; i < itemProperties.length; i++) {
      ...
      for (let j = 0; j < re.claims[itemProperties[i]].length; j++) {
        ...
        if (re.claims[itemProperties[i]][j].mainsnak.datatype === "string") {
          re.claims[itemProperties[i]][j].mainsnak.label = re.claims[itemProperties[i]][j].mainsnak.datavalue.value;
        };
